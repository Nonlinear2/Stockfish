...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && !excludedMove && is_valid(ttData.value)  // Can happen when !ttHit or when access race in probe()
        && ttData.depth > depth - (ttData.value <= beta) - is_decisive(ttData.value)
