...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonusScale_1 = std::min(164 * depth - 102, 1461) * std::max(
              118 * (depth > 5) 
            + 33 * !allNode
            + 164 * ((ss - 1)->moveCount > 8)
            + 133 * (!ss->inCheck && bestValue <= ss->staticEval - 100)
            + 117 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 75)
            + 90 * ((ss - 1)->isTTMove)
            + 86 * (ss->cutoffCnt <= 3)
            + std::min(-(ss - 1)->statScore / 112, 297), 0) * 392 / 32768;

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, bonusScale_1);

        int bonusScale_2 = std::min(153 * depth - 97, 1535) * std::max(
            107 * (depth > 5) 
          + 34 * !allNode
          + 168 * ((ss - 1)->moveCount > 8)
          + 136 * (!ss->inCheck && bestValue <= ss->staticEval - 100)
          + 127 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 75)
          + 83 * ((ss - 1)->isTTMove)
          + 86 * (ss->cutoffCnt <= 3)
          + std::min(-(ss - 1)->statScore / 117, 286), 0) * 208 / 32768;

        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonusScale_2;
        
        int bonusScale_3 = std::min(173 * depth - 99, 1482) * std::max(
            117 * (depth > 5) 
          + 33 * !allNode
          + 154 * ((ss - 1)->moveCount > 8)
          + 137 * (!ss->inCheck && bestValue <= ss->staticEval - 100)
          + 122 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 75)
          + 86 * ((ss - 1)->isTTMove)
          + 84 * (ss->cutoffCnt <= 3)
          + std::min(-(ss - 1)->statScore / 115, 306), 0) * 988 / 32768;
