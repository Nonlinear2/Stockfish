...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonusScale = (121 * (depth > 5) + 164 * ((ss - 1)->moveCount > 8)
                          + 136 * (!ss->inCheck && bestValue <= ss->staticEval - 107)
                          + 123 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 84)
                          + 84 * ((ss - 1)->isTTMove) + 100 * (ss->cutoffCnt <= 3)
