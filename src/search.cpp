...
Value to_corrected_static_eval(const Value v, const int cv, const int simpleEval) {
    return std::clamp(v + cv / (131572 - 1000 * (std::abs(simpleEval) > 962)),
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (ss->ttHit)
    {
        ...
        ss->staticEval = eval = to_corrected_static_eval(unadjustedStaticEval, correctionValue, Eval::simple_eval(pos));
    ...
    else
    {
        ...
        if (ss->ttHit)
        {
            ...
            ss->staticEval = bestValue =
              to_corrected_static_eval(unadjustedStaticEval, correctionValue, Eval::simple_eval(pos));
