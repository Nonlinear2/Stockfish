...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool ttBadSee = ttData.move && !pos.see_ge(ttData.move, 0) && PvNode;
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && ttData.depth >= depth - 3)
            {
                ...
                {
                    int doubleMargin = 249 * PvNode - 194 * (!ttCapture || (ttBadSee && depth < 15));
                    int tripleMargin = 94 + 287 * PvNode - 249 * (!ttCapture || (ttBadSee && depth < 15)) + 99 * ss->ttPv;
