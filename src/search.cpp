...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos, const Move move) {
    auto cv = w.correctionHistory[pos.side_to_move()][pawn_structure_index<Correction>(pos)];
    v += 66 * cv / 512;
    if (move != move.none() && move != move.null())
        v -= 50 * !pos.see_sq(move.to_sq(), 0);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (ss->ttHit)
    {
        ...
        ss->staticEval = eval = to_corrected_static_eval(unadjustedStaticEval, *thisThread, pos, (ss - 1)->currentMove);
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            ss->staticEval = bestValue =
              to_corrected_static_eval(unadjustedStaticEval, *thisThread, pos, (ss - 1)->currentMove);
