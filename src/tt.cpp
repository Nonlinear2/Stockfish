...
struct TTEntry {
    ...
    TTData read() const {
        return TTData{Move(move16),           Value(value16),
                      ...
                      Bound(genBound8 & 0x3), bool(genBound8 & 0x4), should_keep()};
    }

    bool is_occupied() const;
    uint8_t get_depth8() const;
    bool should_keep() const;
    void save(Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8, bool shouldKeep);
...
uint8_t TTEntry::get_depth8() const {
    return should_keep() ? -depth8 - 2*DEPTH_ENTRY_OFFSET - 3 : depth8;
}

bool TTEntry::should_keep() const {
...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8, bool shouldKeep) {

    bool replace = uint16_t(k) != key16 && !should_keep();
    ...
    if (replace || b == BOUND_EXACT || d - DEPTH_ENTRY_OFFSET + 2 * pv > get_depth8() - 4
        || relative_age(generation8))
    {
        ...
        key16     = uint16_t(k);
        depth8    = uint8_t((shouldKeep ? -3-d : d) - DEPTH_ENTRY_OFFSET);
...
void TTWriter::write(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8, bool shouldKeep) {
    entry->save(k, v, pv, b, d, m, ev, generation8, shouldKeep);
}
